...
export function validate(packet) {
  ...
  if (!figures || typeof figures !== "object") errors.push("mainnet_figures is required");
    ...
    if (!Array.isArray(figures.final_submission_hashes) || figures.final_submission_hashes.length !== 3 || new Set(figures.final_submission_hashes.map(h => typeof h === "string" ? h.toLowerCase() : h)).size !== 3 || figures.final_submission_hashes.some(h => !HEX.test(h) || PLACEHOLDER.test(h))) errors.push("final_submission_hashes must contain exactly 3 distinct exact transaction hashes");
  ...
  if (!Array.isArray(packet.contracts) || packet.contracts.length !== REQUIRED_CONTRACT_IDENTITIES.length) errors.push("contracts must list exactly the four required mainnet contract identities");
    ...
    if (new Set(names).size !== REQUIRED_CONTRACT_IDENTITIES.length || REQUIRED_CONTRACT_IDENTITIES.some(name => !names.includes(name))) errors.push("contracts must contain exactly PrismIdentityRegistry, PrismAllocationHelper, PrismVesuLendingHelper, and PrismLayerZeroBase");
...
function runSelfTest() {
  ...
  const packet = {
    ...
    contracts: REQUIRED_CONTRACT_IDENTITIES.map((name, i) => ({ name, address: exact, class_hash: exact, deployment_block: i + 1, constructor_calldata: "0x1", receipt: { tx_hash: exact, block: i + 1, status: "ACCEPTED" }, independent_read: { provider: "independent", address_match: true, class_hash_match: true, verified_at: "2026-01-01T00:00:00Z" } })),

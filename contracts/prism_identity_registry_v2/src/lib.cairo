// PrismIdentityRegistry V2 — exact u256 proof digest registry.
// This is a separate immutable deployment from V1. V1 remains historical and
// is not modified or upgraded. The V2 bind ABI stores the full 256-bit Keccak
// digest as Cairo u256 (low/high u128 limbs), with no felt masking or modulo.
//
// Authority: same accepted controller-only Option A model as V1. V2 adds no
// proxy, replace_class, admin, owner-import, or migration authority.
//
// Deployment posture: immutable, no proxy; fresh identity state at a new address.
...
pub mod PrismIdentityRegistry {
    ...
    /// EVT-EXECUTION-IDENTITY-BOUND — past-tense canonical event (schema v2).
    /// Payload exactly {prism_id, venue, execution_account, proof_digest:u256}
    /// per the V2 event catalogue; the u256 payload serializes as low/high
    /// limbs in event data.
    #[derive(Drop, starknet::Event)]

...
fn digest_value(value: felt252) -> u256 {
    u256 { low: value.try_into().unwrap(), high: 0 }
}
...
#[test]
fn test_8_u256_high_limb_is_part_of_replay_key() {
    let (registry, registry_address) = deploy_registry();
    let controller = address(0x111);
    let base_account = address(0xabc);
    let digest_one = u256 { low: 0x42, high: 0x1 };
    let digest_two = u256 { low: 0x42, high: 0x2 };
    let prism_id = create_as(registry_address, controller);

    bind_as(registry_address, controller, prism_id, base_account, digest_one);
    start_cheat_caller_address(registry_address, controller);
    registry.revoke_binding(prism_id, VENUE_BASE, base_account);
    stop_cheat_caller_address(registry_address);
    // Same low limb but a different high limb is a distinct exact u256 digest.
    bind_as(registry_address, controller, prism_id, base_account, digest_two);
...
fn test_8_bind_success_emits_event_and_records_active_binding() {
    ...
    let controller = address(0x111);
    let base_account = address(0xabc);
    let digest = digest_value(0xd1e5);
    ...
    spy
        .assert_emitted(
            @array![
                (
                    registry_address,
                    prism_identity_registry_v2::PrismIdentityRegistry::Event::BindingRevoked(
                        prism_identity_registry_v2::PrismIdentityRegistry::BindingRevoked {

...
| `POST /api/v1/gifts` | Authenticated workflow caller creates immutable gift terms (`claimId`, payer/sender, refund destination, asset, amount, expiry, commitment). | Local terms are recorded; if a live adapter is configured it receives `createTerms`. No backend caller becomes payer authority. |
| `GET /api/v1/gifts/{claimId}` | Public-safe read of a gift claim projection. | Returns the local lifecycle fact; submission `unknown`/`submitted` is not terminal completion. |
| `POST /api/v1/gifts/{claimId}` | Authenticated explicit `fund`, `mark_claimable`, `claim`, `expire`, or `refund`. `fund` requires payer wallet EIP-712 approval bytes; `claim` requires verifier authorization plus recipient signature bytes; `refund` has no beneficiary parameter and targets contract-stored refund destination. | Funding calls `fund(bytes payerApproval)`; claiming calls `claim(id, recipient, bytes authorization)`; refund calls `refund(id)`. Only a successful exact receipt (`claimId`, tx hash, block number, succeeded) may become terminal. `submitted`/`unknown` are not `completed`; ambiguous claim keeps the nullifier reserved and is poll/reconcile-only. Definite claim failure releases the reservation; persistence failure after submission remains retry/reconcile-required. Absent live adapter is explicit `ERR-062` unavailable. |

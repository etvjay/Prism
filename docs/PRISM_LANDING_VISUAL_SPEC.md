...
**Status:** Design source of truth for the public landing page; implementation not started  
**Date:** 2026-08-20  
...
The bundle declares `prism-quiet-instrument-design-system` and `prism-animated-sidebar-notch` as dependencies, but they are not present in the current local skill catalog. Their tokens and notch geometry must be restored or explicitly recreated before implementation.
...
The supplied image shows a larger dimensional object on a circular pale plinth. The hero skill recommends a 72–96px object footprint. This is a blocking precision decision:
...
Until resolved, preserve the reference image's composition and material language, but do not hard-code a production size.
...
- [ ] Reference image composition is recognizable without copying fake data.
- [ ] Flat Refracted Core is static in navbar/app-shell positions.
- [ ] Living Refracted Core is used for hero and transitions.
- [ ] Hero copy matches the canonical text exactly.
- [ ] Starknet is visually the onchain anchor.
- [ ] `Prism ID → Starknet → venues` and `identities → Prism ID` remain distinct.
- [ ] Home is presented as the destination behind the landing page, not conflated with it.
- [ ] No dashboard screenshot or mock balance appears in the hero.
- [ ] Endpoint states are explicit and truthful.
- [ ] Motion yields to user input and uses shared primitives.
- [ ] Landing and Home feel like one continuous system.
- [ ] Testnet/mainnet status is visible where applicable.

## Implementation gate

Do not begin the landing implementation until the following are resolved:

1. Hero object footprint: reference-scale or 72–96px skill recommendation.
2. Quiet Instrument tokens and animated sidebar-notch dependency.
3. Production source for the flat vector mark and living object; do not use the screenshot as a raster logo.
4. Exact endpoint states for the first demo build.
